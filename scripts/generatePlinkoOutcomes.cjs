...
const SIMULATIONS_PER_ROW = 20000;
const MAX_SAMPLES_PER_BIN = 200; // Cap samples per bin to keep file small
...
function generateOutcomesForRow(rowCount) {
    ...
    for (let i = 0; i < SIMULATIONS_PER_ROW; i++) {
        ...
        const startXPadded = Math.round(pad(startX));
        const bin = simulateBall(startXPadded, paddedPins, sinks, pinRadius, physics);

        if (bin >= 0 && bin < numBins) {
            if (outcomes[bin].length < MAX_SAMPLES_PER_BIN) {
    ...
    // Fill empty bins with targeted brute-force simulations
    for (let i = 0; i < numBins; i++) {
        if (outcomes[i].length === 0) {
            const edgeBias = (i / (numBins - 1)) - 0.5; // -0.5 to 0.5
            for (let attempt = 0; attempt < 2000; attempt++) {
                const startX = WIDTH / 2 + edgeBias * spawnRange * 1.5 + (Math.random() - 0.5) * (spawnRange * 0.3);
                const startXPadded = Math.round(pad(startX));
                const bin = simulateBall(startXPadded, paddedPins, sinks, pinRadius, physics);
                if (bin === i) {
                    outcomes[i].push(startXPadded);
                    console.log(`    ✓ bin ${i} filled after ${attempt + 1} attempts`);
                    break;
                }
            }
            if (outcomes[i].length === 0) {
                console.log(`    ⚠ bin ${i} could not be filled!`);
            }
        }
    }
...
function main() {
    ...
    // Build output file
    const body = JSON.stringify(allOutcomes);

    const outPath = path.resolve(__dirname, '..', 'public', 'plinkoOutcomes.json');
